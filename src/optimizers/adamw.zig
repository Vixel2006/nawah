    ...
    return struct {
        ...
        pub fn step(self: *Self) !void {
            ...
            for (self.params.items, self.m_bufs.items, self.v_bufs.items) |p, m, v| {
                ...
                const wd_t = scalarLike(T, arena, wd) catch continue;
                ...
                const wd_step = functions.mul(T, p, wd_t) catch continue;
                const update = functions.add(T, correction, wd_step) catch continue;
                const step_t = functions.mul(T, update, lr_t) catch continue;
