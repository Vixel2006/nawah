...
void matmul_relu_cpu_forward_direct(
  ...
  matmul_relu_cpu_forward_float_contig_kernel((const float *)pa.data, (const float *)pb.data, c_data, batches, M, K, N, alpha);

  tensor_pack_release(&pa);
  tensor_pack_release(&pb);
}

void matmul_relu_cpu_backward_direct(
    const float *a_data, const u64 *a_shape, const u64 *a_strides, u64 a_ndim,
  ...
  relu_grad_modulate_contig_kernel((const float *)dc_tensor.data, (const float *)output.data, dc_mod,
                                   num_elements, alpha);
