...
__device__ static size_t calculate_linear_index_cuda(const size_t* coords, const size_t* strides,
                                                     size_t ndim)
{
    size_t index = 0;
...
template <typename T>
__global__ void broadcast_kernel_cuda(const T* input_data, const size_t* input_shape,
                                      ...
                                      size_t output_ndim)
{
    ...
    if (idx < total_output_elements)
    {
        ...
        size_t src_linear_index =
            calculate_linear_index_cuda(output_coords, padded_input_strides, output_ndim);
...
void cuda_broadcast_kernel(const void* input_data, const size_t* input_shape_host,
                           const size_t* input_strides_host, size_t input_ndim, void* output_data,
                           const size_t* output_shape_host, size_t output_ndim, size_t item_size)
{
    size_t total_output_elements = 1;
    for (size_t i = 0; i < output_ndim; ++i)
    {
    ...
    PLAST_CUDA_CHECK(cudaMemcpy(input_shape_dev, input_shape_host, input_ndim * sizeof(size_t),
                                cudaMemcpyHostToDevice));
    PLAST_CUDA_CHECK(cudaMemcpy(input_strides_dev, input_strides_host, input_ndim * sizeof(size_t),
                                cudaMemcpyHostToDevice));
    PLAST_CUDA_CHECK(cudaMemcpy(output_shape_dev, output_shape_host, output_ndim * sizeof(size_t),
                                cudaMemcpyHostToDevice));

    // Launch kernel based on item_size (dtype)
    if (item_size == sizeof(float))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const float*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (float*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(double))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const double*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (double*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(int8_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const int8_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (int8_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(int16_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const int16_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (int16_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(int32_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const int32_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (int32_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(int64_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const int64_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (int64_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(uint8_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const uint8_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (uint8_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(uint16_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const uint16_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (uint16_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(uint32_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const uint32_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (uint32_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(uint64_t))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const uint64_t*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (uint64_t*) output_data, output_shape_dev, output_ndim);
    }
    else if (item_size == sizeof(bool))
    {
        broadcast_kernel_cuda<<<gridSize, blockSize>>>(
            (const bool*) input_data, input_shape_dev, input_strides_dev, input_ndim,
            (bool*) output_data, output_shape_dev, output_ndim);
    }
    else
    {
