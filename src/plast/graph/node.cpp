...
const tensor::Tensor& Node::get_cached_value() const
{
    if (!cached_value_.has_value())
    {
        throw std::runtime_error(
            "Attempted to get cached value from a node that has not been computed.");
    }
    return *cached_value_; // Return a const reference
}

bool Node::has_cached_value() const { return cached_value_.has_value(); }

void Node::clear_cached_value() { cached_value_.reset(); }
