                           ...
                           const std::vector<const tensor::Tensor*>& inputs) const
{
    ...
    if (input->requires_grad())
    {
        ...
        case core::DType::FLOAT32:
            if (full_reduction_)
            {
                plast_cpu_min_full_reduction_backward_float(
            ...
            {
                plast_cpu_min_reduction_dim_backward_float(
                    grad_input.data_as<float>(), grad_output.data_as<const float>(),
                    inputs[0]->data_as<const float>(), output.data_as<const float>(),
        ...
        case core::DType::INT32:
            if (full_reduction_)
            {
                plast_cpu_min_full_reduction_backward_int32(
            ...
            {
                plast_cpu_min_reduction_dim_backward_int32(
                    grad_input.data_as<int32_t>(), grad_output.data_as<const int32_t>(),
                    inputs[0]->data_as<const int32_t>(), output.data_as<const int32_t>(),
                    inputs[0]->shape().data(), inputs[0]->shape().size(), output.shape().data(),
                    output.shape().size(), dim_);
            ...
            break;
        default:
            throw std::runtime_error("Unsupported DType for Min backward on CPU.");
        }
                            ...
                            const std::vector<const tensor::Tensor*>& inputs) const
{
    ...
    if (input->requires_grad())
    {
        ...
        case core::DType::FLOAT32:
            if (full_reduction_)
            {
                plast_cuda_min_full_reduction_backward_float(
                    grad_input.data_as<float>(), grad_output.data_as<const float>(),
                    inputs[0]->data_as<const float>(), output.data_as<const float>(),
                    inputs[0]->shape().data(), inputs[0]->shape().size());
            }
            else
            {
                throw std::runtime_error(
                    "CUDA Min reduction dim float backward operation not yet implemented.");
            }
            break;
        case core::DType::INT32:
            if (full_reduction_)
            {
                plast_cuda_min_full_reduction_backward_int32(
                    grad_input.data_as<int32_t>(), grad_output.data_as<const int32_t>(),
                    inputs[0]->data_as<const int32_t>(), output.data_as<const int32_t>(),
                    inputs[0]->shape().data(), inputs[0]->shape().size());
            }
            ...
            {
                throw std::runtime_error(
                    "CUDA Min reduction dim int32 backward operation not yet implemented.");
            }
            break;
        default:
            throw std::runtime_error("Unsupported DType for Min backward on CUDA.");
        }
        input_grads.push_back(std::move(grad_input));
    }
    else
