...
tensor::Tensor MeanOperation::execute_cpu(const std::vector<const tensor::Tensor*>& inputs) const
{
    ...
    case core::DType::FLOAT32:
        if (full_reduction_)
        {
            ...
            {
                plast_cpu_mean_full_reduction_strided_float(
        ...
        else
        {
            ...
            {
                plast_cpu_mean_reduction_dim_strided_float(
                    input.data_as<const float>(), output.data_as<float>(), input.shape().data(),
    ...
    case core::DType::INT32:
        if (full_reduction_)
        {
            ...
            {
                plast_cpu_mean_full_reduction_strided_int32(
        ...
        else
        {
            ...
            {
                plast_cpu_mean_reduction_dim_strided_int32(
                    input.data_as<const int32_t>(), output.data_as<int32_t>(), input.shape().data(),
                    input.shape().size(), input_strides, output_shape_arr, output.ndim(), dim_);
            }
        }
