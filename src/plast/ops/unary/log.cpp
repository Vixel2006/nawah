...
tensor::Tensor LogOperation::execute_cpu(const std::vector<const tensor::Tensor*>& inputs) const {
    ...
    switch (dtype) {
        case core::DType::FLOAT32:
            plast_cpu_log_kernel_float(output.data_as<float>(), input.data_as<const float>(), num_elements);
            break;
...
tensor::Tensor LogOperation::execute_cuda(const std::vector<const tensor::Tensor*>& inputs) const {
    ...
    switch (dtype) {
        case core::DType::FLOAT32:
            plast_cuda_log_kernel_float(output.data_as<float>(), input.data_as<const float>(), num_elements);
            break;
        // Add more types as needed
        default:
