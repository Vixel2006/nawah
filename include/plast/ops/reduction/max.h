...
class MaxOperation : public BaseOperation
{
  public:
    MaxOperation(int dim, bool keepdim) : dim_(dim), keepdim_(keepdim), full_reduction_(false) {}
    MaxOperation(bool full_reduction) : full_reduction_(full_reduction) {}

    const std::string& name() const override
    {
        static std::string op_name = "max";
        return op_name;
    }
