...
class SumOperation : public BaseOperation
{
  public:
    SumOperation(int dim, bool keepdim) : dim_(dim), keepdim_(keepdim), full_reduction_(false) {}
    SumOperation(bool full_reduction) : full_reduction_(full_reduction) {}

    const std::string& name() const override
    {
        static std::string op_name = "sum";
        return op_name;
    }
